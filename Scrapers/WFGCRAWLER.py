    ...
    if soup.find('div', attrs={'id': 'alertBox'}) != None:
        ...
        for td in table[0].find_all('td'):
            ...
            if urlNotFound == 'N':
                ...
                #print(found_flag)
    print(Wg.Process_has_found_member)
    print('Process_has_found_member ' + Wg.Process_has_found_member + ' for ' + search_name[0] + ' ' + search_name[1])
    if Wg.Process_has_found_member != 'Y':
        url = 'EMPTY'
        found_flag = search_page(url, search_name, Pcode)
        print(found_flag)
    ...
    if url != 'EMPTY':
        ...
        if soup.find('div', attrs={'id': 'ourTeams'}) != None: # and group_exists != 'Y':
            ...
            for groups in soup.find('div', attrs={'id': 'ourTeams'}).find('ul').find_all('li'):
                ...
                grp_found = Search_for_Group(grp_url, search_name, grp_name)
